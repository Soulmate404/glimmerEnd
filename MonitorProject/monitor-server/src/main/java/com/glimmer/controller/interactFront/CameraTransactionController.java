...
public class CameraTransactionController {
    ...
    private final CameraTransactionService cameraTransactionService;
    @Autowired
    public CameraTransactionController(CameraTransactionService cameraTransactionService) {
        this.cameraTransactionService = cameraTransactionService;
    }
...
*/
    ...
    public Result<StatusVO> AddCamera(@RequestBody AddCameraDTO addCameraDTO) {
        log.info("add camera : {}",addCameraDTO);//记录日志

        cameraTransactionService.AddCamera(addCameraDTO);//调用业务层相关方法

        StatusVO statusVO= StatusVO.builder()
                .status(0)
                .message(MessageConstant.SUCCESS).build();
        return Result.success(statusVO,"add success");
        //构造对象返回
...
*/
    ...
    public Result<StatusVO> DeleteCamera(@RequestBody DeleteCameraDTO deleteCameraDTO) {
        log.info("delete camera: {}",deleteCameraDTO);//记录日志

        cameraTransactionService.deleteCamera(deleteCameraDTO);//调用业务层相关方法

        StatusVO statusVO= StatusVO.builder()
                .status(0)
                .message(MessageConstant.SUCCESS).build();
        return Result.success(statusVO,MessageConstant.SUCCESS);//构造对象返回
...
*/
    ...
    public Result<GetCameraVO> GetCamera(@RequestBody GetCameraDTO getCameraDTO) {
        log.info("get camera: {}",getCameraDTO);//记录日志
        ...
        GetCameraVO getCameraVO= GetCameraVO.builder()
                ...
                .urls(cameraTransactionService.GetCamera(getCameraDTO)).build();

        return Result.success(getCameraVO,MessageConstant.SUCCESS);//构造对象返回
...
*/
    ...
    public Result<StatusVO> UpdateCamera(@RequestBody UpdateCameraDTO updateCameraDTO) {
        log.info("update info: {}",updateCameraDTO);//记录日志

        cameraTransactionService.updateCamera(updateCameraDTO);//调用业务层相关方法
