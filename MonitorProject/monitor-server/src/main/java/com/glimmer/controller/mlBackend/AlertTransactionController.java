...
import com.glimmer.dto.AddAlertDTO;
import com.glimmer.dto.AlertVideoAndPhotoDTO;
import com.glimmer.dto.AlertVideoDTO;
...
import com.glimmer.vo.GetAlertVideoVO;
...
public class AlertTransactionController {
    ...
    */
    @PostMapping("/info")
    public Result<StatusVO> AlertInfo( AddAlertDTO alertPhoto) {
        log.info("文件上传：{}", alertPhoto);
        alertTransactionService.uploadPhoto(alertPhoto);
    ...
    */
    @PostMapping("/video")
    public Result<StatusVO> AlertVideo(AlertVideoDTO alertVideo) {
        log.info("文件上传：{} ", alertVideo);
        alertTransactionService.uploadVideo(alertVideo);
        StatusVO statusVO = StatusVO.builder()
                .status(GetStatusConstant.SUCCESS)
                .message("报警信息存储" + MessageConstant.SUCCESS)
                .build();
    ...
    public Result<GetAlertVO> getAlert(@RequestBody GetAlertDTO getAlertDTO){
        log.info("获取报警数据： {} ", getAlertDTO);
        GetAlertVO getAlertVO=GetAlertVO.builder()
                .status(GetStatusConstant.SUCCESS)
                .message(MessageConstant.SUCCESS)
                .infos(alertTransactionService.getAlert(getAlertDTO))
                .build();
        return Result.success(getAlertVO, MessageConstant.SUCCESS);

    }
    @GetMapping("video")
    public Result<GetAlertVideoVO> getAlertSrc(@RequestBody AlertVideoAndPhotoDTO alertVideoAndPhotoDTO){
        log.info("视频数据获取 {} ", alertVideoAndPhotoDTO);
        GetAlertVideoVO getAlertVideoVO=GetAlertVideoVO.builder()
                .status(GetStatusConstant.SUCCESS)
                .message(MessageConstant.SUCCESS)
                .video(alertTransactionService.downloadFile(alertVideoAndPhotoDTO))
                .build();
        return Result.success(getAlertVideoVO, MessageConstant.SUCCESS);
